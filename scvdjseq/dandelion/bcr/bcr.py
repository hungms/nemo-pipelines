...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="human",
        help=(
            "Whether to process for human or mouse, for annotations and isotype "
            + 'assignment. Defaults to "human".'
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If passed, skips reassigning allels and germline sequence "
            + "identification."
        ),
    )
    parser.add_argument(
    ...
    # convert org to lower case for compatibility, and ensure it's in human/mouse
    args.org = args.chain.lower()
    if args.org not in ["human", "mouse"]:
        raise ValueError("org must be human or mouse")
    return args
...
def main():
    ...
    logg.info(
        ...
        deep=(
            ...
            f"    --meta = {args.meta}\n"
            f"    --org = {args.org}\n"
            f"    --skip_reassign_alleles = {args.skip_reassign_alleles}\n"
            f"    --chain = {args.chain}\n"
    ...
    if args.chain == "ig":
        ...
        if not args.skip_reassign_alleles:
        ...
        for s in samples:
            ddl.store_germline_reference(
                s
                + "/dandelion/"
                + str(args.file_prefix)
                + "_contig_dandelion.tsv",
                germline = "/share/database/germlines/imgt/" + str(args.org) + "/vdj/",
                org = args.org,)
