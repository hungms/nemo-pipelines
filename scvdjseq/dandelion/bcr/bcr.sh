...
export DATA=${PRJ}/dandelion/20240621_bcr/
mkdir -p ${DATA}

export LOGS=/flask/scratch/caladod/hungm/${accession}/logs/scvdjseq/dandelion/bcr/
mkdir -p ${LOGS}

### run singularity pipeline
export sif=/camp/home/hungm/working/Matthew/library/pythonlib/dandelion/sc-dandelion_latest.sif
cd ${DATA}
singularity run -B ${DATA},${LOGS} $sif python ${LOGS}/bcr.py \
	--org mouse \
	--file_prefix all \
	--meta $LOGS/meta.csv \
