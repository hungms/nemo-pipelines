...
my $reference_fasta = '/camp/stp/babs/working/goldstr2/projects/caladod/anqi.xu/DN19023-ax343/reference.fasta';

my $reference_id = '12:113428106-113428538';

my $script_root = '/camp/stp/babs/working/goldstr2/projects/caladod/anqi.xu/DN19023-ax343/task_5';
...
my $software_versions_file = $script_root.'/software_versions.csv';  ## this file lists the different camp modules that we'll need.  Here we just check it exists (that I created it properly)
...
open (my $dfh, '<', $script_root.'/design.csv'); # reads in the design file we made
...
for my $lines (@design_array) {
	...
	if (exists $counter->{$sample_name}->{'fq_2'}) {
		...
		$counter->{$sample_name}->{'fq_2'} = [$fastq_1];
	};
};
...
sub merge_reads {
	...
	unless (-e $script_root.'/merged_reads/'.$sample.'.fastq.gz') {
		...
		my $merge = 'bbmerge.sh in='.$f1.' '.'in2='.$f2.' out='.$script_root.'/merged_reads/'.$sample.'.fastq.gz';
